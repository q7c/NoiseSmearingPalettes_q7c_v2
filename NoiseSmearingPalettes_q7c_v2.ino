...
cFastLEDSingleMatrix<MATRIX_WIDTH, MATRIX_HEIGHT, MATRIX_TYPE> leds;
cFastLEDSingleMatrix<MATRIX_WIDTH, MATRIX_HEIGHT, MATRIX_TYPE> leds2;
...
void MoveX(byte delta) {
  ...
  for (int y = 0; y < kMatrixHeight; y++) {
    ...
    for (int x = 0; x < kMatrixWidth - delta; x++) {
      leds2(x, y) = leds(x + delta, y);
    }
    for (int x = kMatrixWidth - delta; x < kMatrixWidth; x++) {
      leds2(x, y) = leds(x + delta - kMatrixWidth, y);
    }
  }
...
void MoveY(byte delta) {
  ...
  for (int x = 0; x < kMatrixWidth; x++) {
    ...
    for (int y = 0; y < kMatrixHeight - delta; y++) {
      leds2(x, y) = leds(x, y + delta);
    }
    for (int y = kMatrixHeight - delta; y < kMatrixHeight; y++) {
      leds2(x, y) = leds(x, y + delta - kMatrixHeight);
    }
  }
...
void MoveFractionalNoiseX(byte amt = 16) {
  ...
  for (uint8_t y = 0; y < kMatrixHeight; y++) {
    ...
    for (uint8_t x = 1; x < kMatrixWidth; x++) {
      ...
      leds(x, y) = PixelA + PixelB;
      //leds2(x, y) = 300;
    }

    PixelA = leds2(0, y);
    PixelB = leds2(kMatrixWidth - 1, y);
...
void MoveFractionalNoiseY(byte amt = 16) {
  ...
  for (int x = 0; x < kMatrixWidth; x++) {
    ...
    for (int y = kMatrixWidth - delta; y < kMatrixWidth; y++) {
      leds2(x, y) = leds(x, y + delta - kMatrixWidth);
    }
  }
  //CopyBack();
...
void MultipleStream4() {
  ...
  leds( 15, 15) += CHSV(millis(), 255, 255);
...
void PaletteSmear() {
  ...
  for (uint8_t y = 0; y < kMatrixHeight; y++) {
    for (uint8_t x = 0; x < kMatrixWidth; x++) {
      leds(x, y) += ColorFromPalette(currentPalette, x * 8, y * 8 + 7, LINEARBLEND);
    }
  }
